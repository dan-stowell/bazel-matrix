...
Each museum project gets a //wild/<project> package whose `:build` and `:test`
targets run the project's *upstream* build/test (its pinned source, its own
MODULE/BUILD, no overlays, no injected toolchain) inside the //wild/image
container via wild/run.sh:

    bazel run //wild/image:load        # once: build + load the CI image
    bazel run //wild/cpu_features:build
    bazel run //wild/cpu_features:test
